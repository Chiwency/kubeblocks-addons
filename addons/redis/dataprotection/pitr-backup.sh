...
function archive_pair_files() {
  ...
  tar -cvf - "$(generate_backup_manifest)" -C "${DATA_DIR}" \
    ...
    "users.acl" "nodes.conf" | datasafed push -z zstd - "${target_file}"
...
function update_aof_file() {
  ...
  if [ $(stat -c %Y "${base_file}") -gt ${global_aof_last_modify_time} ]; then
    ...
    [ -f "${DATA_DIR}/nodes.conf" ] && datasafed push "${DATA_DIR}/nodes.conf" "${backup_files_prefix}.dir/nodes.conf"
...
function save_backup_status() {
  [ "${MODE}" == "cluster" ] && export DATASAFED_BACKEND_BASE_PATH="$(dirname $DP_BACKUP_BASE_PATH)"
  total_size=$(datasafed stat / | grep TotalSize | awk '{print $2}')
  [ "${MODE}" == "cluster" ] && export DATASAFED_BACKEND_BASE_PATH="$DP_BACKUP_BASE_PATH"
